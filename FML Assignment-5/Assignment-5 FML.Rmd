...
“ISLR”, “caret”, “dplyr”, “tidyverse”, “factoextra”, “ggplot2”, “proxy”, “NbClust”, “ppclust”, “dendextend”, “cluster”, and “tinytex” will be loaded for this problem.
...
cereal <- read.csv("C:/Users/dell/Downloads/Cereals.csv")
```
...
#In this case, normalizing the data would be improper. This is because the specific cereal sample under investigation determines how nutrition data from cereals should be scaled or normalized. Because of this, the data set that was collected might only contain cereals that are very high in sugar but low in iron, fiber, and other nutrients. It is impossible to estimate the nutritional value of cereal for a child after the data inside the sample set is scaled or normalized. An insensitive observer may conclude that a cereal with an iron score of 0.999 gives a child almost all of the iron they require, but it might actually be the best of the worst in the sample set, offering very little to no iron.


#Therefore, converting the data into a ratio to a child's daily recommended intake of calories, fiber, carbs, and other nutrients would be a preferable way to preprocess the data. By doing this, analysts would be able to evaluate clusters more intelligently and prevent a small number of significant variables from overriding distance computations. When analyzing the clusters, an analyst may use the cluster averages to determine how much of a student's daily nutritional needs would come from XX cereal. This would allow the employees to choose "healthy" cereal clusters with greater knowledge.
